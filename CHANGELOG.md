...
- **One dead browser aborted the entire scan (OPS-1):** `createContext` and `newPage` sat outside the per-page `try`, so when the browser died — `browserContext.newPage: Target page, context or browser has been closed` — the task rejected, `Promise.all` rejected, and every remaining page was lost. Page setup now happens inside the try, so a failure is recorded against that one URL and the scan continues. On serverless the browser is also recycled per page, since `@sparticuz/chromium` runs `--single-process` where a renderer crash takes the whole browser down; a disconnected browser is replaced rather than reused. Two further faults surfaced while testing this: the browser was being thrown away after *any* page error, including a plain navigation timeout that leaves it perfectly usable, and concurrent pages each launched their own browser because acquisition was not serialised.
...
- **Chromium on Vercel (OPS-1):** stock Playwright browsers exit 127 on Amazon Linux (`error while loading shared libraries: libnspr4.so`), and `playwright install --with-deps` cannot help because it shells out to an absent `apt-get`. `@sparticuz/chromium` supplies a build with those libraries bundled; `launchBrowser` picks it under `VERCEL`/`AWS_LAMBDA_FUNCTION_NAME` and Playwright's own browser locally.
